...
import edu.ntu.eee.csn.ism.egene.exception.WordNetUtilException;

public class WordNet {

	private final Logger LOGGER = Logger.getLogger(WordNet.class);
	...
	private void init() {
		...
		String wordnetProp = this.config.getEgeneCfgDirName()
				+ File.separator
				+ this.config.getString(ConfigParam.WORDNET_PROPERTIES_FILE);
		File wordnetFilePropertiesXml = new File(wordnetProp);
		if (!wordnetFilePropertiesXml.exists()) {
			throw new WordNetUtilException("CAN'T find "
					+ ConfigParam.WORDNET_PROPERTIES_FILE.getName() + "=["
					+ wordnetFilePropertiesXml.getAbsolutePath() + "]");
		} else {
		...
		try {
			JWNL.initialize(new FileInputStream(wordnetFilePropertiesXml));
			this.dict = Dictionary.getInstance();
		} catch (FileNotFoundException e) {
			throw new WordNetUtilException(e.getMessage(), e);
	...
	public boolean isWord(String word) {
		...
		try {
			indexWordSet = this.dict.lookupAllIndexWords(word);
		} catch (JWNLException e) {
		}
	...
	public boolean isAdjective(String word) {
		...
		try {
			indexWord = this.dict.lookupIndexWord(POS.ADJECTIVE, word);
		} catch (JWNLException e) {
		}
	...
	public boolean isAdverb(String word) {
		...
		IndexWord indexWord = null;
		try {
			indexWord = this.dict.lookupIndexWord(POS.ADVERB, word);
		} catch (JWNLException e) {
		}
	...
	public boolean isNoun(String word) {
		...
		IndexWord indexWord = null;
		try {
			indexWord = this.dict.lookupIndexWord(POS.NOUN, word);
		} catch (JWNLException e) {
		}
	...
	public String getLemma(String word, POS pos) {
		...
		IndexWord indexWord = null;
		try {
			indexWord = this.dict.lookupIndexWord(pos, word);
		} catch (JWNLException e) {
		}
		if (indexWord == null)
			return null;
	...
	public String getCompoundLemma(String word, POS pos) {
		...
		String lemma = this.getLemma(word, pos);
		if (lemma == null)
			return null;
	...
	private Synset getCommonSynset(String word, POS pos) {
		...
		Synset synset = null;
		try {
			IndexWord indexWord = this.dict.lookupIndexWord(pos, word);
			if (indexWord == null)
				return null;
	...
	private String[] getLemmas(Synset[] synsets) {
		...
		for (Synset synset : synsets) {
			String[] lemmas = this.getLemmas(synset);
	...
	public String[] getHypernyms(String word, POS pos) {
		Synset synset = this.getCommonSynset(word, pos);
		if (synset == null)
			return null;
		...
		return this.getLemmas(hypernyms);
	}
	...
	private Synset[] getHypernymSynsets(Synset synset) {
		...
		return this.getSynsets(hypernyms);
	}
	...
	public String[] getHyponyms(String word, POS pos) {
		...
		return this.getLemmas(hyponyms);
	}
	 ...
	 *            POS of the synset
	 * @param offset
	 *            offset of the synset
	 * @return hyponyms or <code>null</code> if lookup failed
	 */
	public String[] getHyponyms(POS pos, long offset) {
		Synset synset = null;
		try {
			synset = this.dict.getSynsetAt(pos, offset);
		} catch (JWNLException e) {
		}
		if (synset == null)
			return null;
	...
	public String[] getNounHyponyms(long offset) {
		return this.getHyponyms(POS.NOUN, offset);
	}
	...
	private Synset[] getHyponymSynsets(Synset synset) {
		...
		return this.getSynsets(hyponyms);
	}
	...
	public String[] getEntailing(String verb) {
		...
		return this.getLemmas(entailing);
	}
	...
	private Synset[] getEntailingSynsets(Synset synset) {
		...
		return this.getSynsets(entailing);
	}
	...
	public String[] getCausing(String verb) {
		Synset synset = this.getCommonSynset(verb, VERB);
		if (synset == null)
			return null;
	...
	private Synset[] getCausingSynsets(Synset synset) {
		...
		return this.getSynsets(causing);
	}
	...
	public String[] getMembersOf(String noun) {
		Synset synset = this.getCommonSynset(noun, NOUN);
		if (synset == null)
			return null;
		...
		return this.getLemmas(membersOf);
	}
	...
	private Synset[] getMemberOfSynsets(Synset synset) {
		...
		return this.getSynsets(membersOf);
	}

	/**
	 * Looks up substance holonyms of the given noun, assuming that it is used
	...
	public String[] getSubstancesOf(String noun) {
		...
		return this.getLemmas(substancesOf);
	}
	...
	private Synset[] getSubstanceOfSynsets(Synset synset) {
		...
		try {
			substancesOf = PointerUtils.getInstance().getSubstanceHolonyms(
					synset);
		} catch (JWNLException e) {
		}
		if (substancesOf == null)
		...
		return this.getSynsets(substancesOf);
	}
	...
	private Synset[] getPartOfSynsets(Synset synset) {
		...
		return this.getSynsets(partsOf);
	}
	...
	public String[] getHaveMember(String noun) {
		...
		return this.getLemmas(haveMember);
	}
	...
	private Synset[] getHasMemberSynsets(Synset synset) {
		...
		return this.getSynsets(haveMember);
	}

	/**
	 * Looks up substance meronyms of the given noun, assuming that it is used
	 * in its most common sense.
	 * 
	 * @param noun
	 *            a noun
	...
	public String[] getHaveSubstance(String noun) {
		...
		return this.getLemmas(haveSubstance);
	}
	...
	private Synset[] getHasSubstanceSynsets(Synset synset) {
		...
		try {
			haveSubstance = PointerUtils.getInstance().getSubstanceMeronyms(
					synset);
		} catch (JWNLException e) {
		}
		if (haveSubstance == null)
		...
		return this.getSynsets(haveSubstance);
	}
	...
	public String[] getHavePart(String noun) {
		...
		return this.getLemmas(havePart);
	}
	...
	private Synset[] getHasPartSynsets(Synset synset) {
		...
		return this.getSynsets(havePart);
	}
	...
	public Map<String, Double> expandEvent(String event) {
		...
		while (pathLength++ < MAX_PATH_LENGTH && synsets.size() > 0) {
			...
			Synset[] currSynsets = synsets.keySet().toArray(
					new Synset[synsets.size()]);
		...
		for (Synset synset : expanded.keySet()) {
			...
			if (weight == 1)
				weight = SYNONYM_WEIGHT; // direct synonyms
		...
		for (String expansion : expansions.keySet())
			if (expansion.equalsIgnoreCase(event))
				dropped.add(expansion);
		for (String expansion : dropped)
	...
	public Map<String, Double> expandEntity(String entity) {
		...
		List<String> dropped = new ArrayList<String>();
		for (String expansion : expansions.keySet())
			if (expansion.equalsIgnoreCase(entity))
				dropped.add(expansion);
		for (String expansion : dropped)
			expansions.remove(expansion);
	 ...
	 *            a modifier
	 * @param pos
	 *            its part of speech: <code>POS.ADJECTIVE</code> or
	...
	public Map<String, Double> expandModifier(String modifier, POS pos) {
		...
		List<String> dropped = new ArrayList<String>();
		for (String expansion : expansions.keySet())
			if (expansion.equalsIgnoreCase(modifier))
				dropped.add(expansion);
		for (String expansion : dropped)
			expansions.remove(expansion);
