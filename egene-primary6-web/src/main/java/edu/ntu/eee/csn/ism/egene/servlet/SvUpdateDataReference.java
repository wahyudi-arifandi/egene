...
@WebServlet(description = "Update data reference of eGENE", urlPatterns = { "/SvUpdateDataReference" })
public class SvUpdateDataReference extends HttpServlet {

	private static final long serialVersionUID = -4578875876383297688L;
	private static Logger LOGGER = Logger
			.getLogger(SvUpdateDataReference.class);
	...
	protected void doGet(HttpServletRequest request,
		...
		this.updateDataReference(request, response);
	...
	private void updateDataReference(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {

		// validate id
		String idKey = Constant.TABLE_REF_C_ID.toString();
		String idVal = request.getParameter(idKey);
		if (StringUtils.isEmpty(idVal) || !StringUtils.isNumeric(idVal)) {
			response.getOutputStream().print(
					ServletResponse.fail(SvUpdateDataReference.class.getName()));
		...
		try {
			...
			String sql = "UPDATE " + tableNameVal + " SET " + valueKey + "='"
					+ valueVal + "' WHERE " + idKey + "=" + idVal;
			if (LOGGER.isDebugEnabled())
			...
			String res = ServletResponse.success(SvUpdateDataReference.class.getName());
